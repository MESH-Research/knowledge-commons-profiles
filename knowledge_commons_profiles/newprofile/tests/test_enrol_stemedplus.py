...
class EnrolStemedplusTests(TestCase):
    """``enrol_stemedplus`` flags users via ``Profile.role_overrides``."""
    ...
    def test_blank_lines_are_skipped(self):
        ...
        flagged = Profile.objects.filter(
            role_overrides__contains=["STEMED+"]
        ).count()
    ...
    def test_idempotent_when_already_enrolled(self):
        ...
        self._run(path)
        self.alice.refresh_from_db()
        self.assertEqual(self.alice.role_overrides.count("STEMED+"), 1)
    ...
    def test_membership_api_reflects_enrolment(self):
        ...
        REST API) records STEMED+ after the override is written."""
        path = _write_file(["alice@example.test"])
        self._run(path)
        self.alice.refresh_from_db()
        memberships = get_external_memberships(self.alice)
        self.assertTrue(memberships["STEMED+"])
    ...
    def test_enrolling_in_stemedplus_does_not_grant_hastac(self):
        path = _write_file(["alice@example.test"])
        self._run(path)
        self.alice.refresh_from_db()
        memberships = get_external_memberships(self.alice)
        self.assertTrue(memberships["STEMED+"])
        self.assertFalse(memberships["HASTAC"])

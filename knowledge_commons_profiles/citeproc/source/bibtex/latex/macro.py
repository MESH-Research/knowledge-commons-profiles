...
from . import CLOSE_SCOPE
from . import OPEN_SCOPE
from . import START_MACRO
from . import eat_whitespace
from . import parse_argument
from . import parse_macro_name

__all__ = ["MACROS", "Macro", "NewCommand"]
...
class NewCommand(MacroBase):
    ...
    def parse_arguments_and_expand(self, tokens, macros):
        ...
        for i in range(10):
            definition = definition.replace(f"#{i + 1}", "{" + str(i) + "}")
...
class Combining(Macro):
    ...
    DOTTED_CHARS = {"ı": "i", "ȷ": "j"}  # noqa: RUF001

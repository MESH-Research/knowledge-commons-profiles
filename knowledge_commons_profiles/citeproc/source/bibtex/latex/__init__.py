...
def parse_latex(string, macros=None):
    """
    Parse a LaTeX string into BibTeX
    """
    if macros is None:
        macros = {}
...
Token = namedtuple("Token", ["type", "value"])  # noqa: PYI024
...
class ScopeClosingError(Exception):
    """
    Exception raised when a closing parenthesis is encountered
    """
...
def dispatch(tokens, macros, level=0):
    while True:
        try:
            ...
            if level > 0:
                warn(
                    f"Unbalanced parenthesis in '{tokens.string}'",
                    stacklevel=2,
                )
            break
...
def parse_macro_name(tokens):
    ...
    if name.isalpha():
        while (
            tokens.peek().type == CHARACTER and tokens.peek().value.isalpha()
        ):
...
def handle_math(tokens):
    ...
    for token in tokens:
        if token.type == START_MACRO:
            output += token.value
            token = next(tokens)  # noqa:PLW2901

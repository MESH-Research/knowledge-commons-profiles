...
from . import NAMES
from . import NUMBERS
from . import PRIMARY_DIALECTS
from .source import DateRange
from .source import LiteralDate
from .source import VariableError
...
class CitationStylesElement(SomewhatObjectifiedElement):
    ...
    def loc(self):
        ...
        for i, node in enumerate(full_xpath.split("/")[1:]):
            ...
            tree.append(
                "{:>4}: {}<{}{}>".format(
                    element.sourceline, i * "  ", tag, attribs
                )
            )
    ...
    # TODO: Locale methods
    def get_term(
        self, name, form=None, fallback_locale=True, zero_padded=False
    ):
            ...
            if main_lg:
                ...
                for locale in locales[1::]:
                    if locale.get(lg_key, main_lg) == main_lg:
                        new_locales.append(locale)  # noqa: PERF401
                locales = new_locales
...
class Style(CitationStylesElement):
    def set_locale_list(self, output_locale, validate=True):
        ...
        def add_instyle_locale(locale):
            expr = (
                f'./cs:locale[@xml:lang="{locale}"]'
                if locale
                else "./cs:locale[not(@xml:lang)]"
            )
...
class Formatted:
    def format(self, string):
        ...
        return self.vertical_align(text)
...
class TextCased:
    ...
    def case(self, text, language=None):  # noqa: PLR0912, C901
        ...
        if text_case is not None:
            ...
            if text_case == "lowercase":
                ...
                for word in text.words():
                    new_word = word
                    new_word = new_word.capitalize_first()
                    output.append(word)
                text = " ".join(output)
                ...
                for word in text.words():
                    new_word = word
                    if new_word.islower() and (
                        str(new_word) not in self._stop_words
                        or prev in (":", ".")
                    ):
                        new_word = new_word.capitalize_first()
                    elif new_word.islower():  # Lowercase stop words
                        new_word = new_word.soft_lower()
                    prev = new_word[-1]
                ...
                for i, word in enumerate(text.words()):
                    new_word = word
                    if not text.isupper() and not new_word.isupper():
                        new_word = new_word.soft_lower()
                    if i == 0:
                        new_word = new_word.capitalize_first()
                    output.append(new_word)
...
class Sort(CitationStylesElement):
    def sort(self, items, context):
        ...
        def multi_key_sort(items, keys, descending):
            lst = zip(items, *keys, strict=False)
            comparers = [
                (itemgetter(i + 1), descending[i]) for i in range(len(keys))
            ]
    ...
    def sort_keys(self, items, context):
        if "variable" in self.attrib:
            ...
            if variable in NAMES:
                sort_keys = [
                    self._format_name(item, variable) for item in items
                ]
                ...
                sort_keys = [
                    self._format_number(item, variable) for item in items
                ]
            ...
            sort_keys = self._handle_macro(context, items)

        return sort_keys

    def _handle_macro(self, context, items):
        ...
        for item in items:
            layout.repressed = {}
            sort_key = macro.render(
                item, context=context, sort_options=sort_options
            )
    ...
    def _format_name(self, item, variable):
        ...
        if names is not None:
            ...
            for name in names:
                demote_ndp = (
                    self.get_root()
                    .get("demote-non-dropping-particle", "display-and-sort")
                    .lower()
                )
...
class FormatNumber:
    def _process(self, value, variable):
        page_range_delimiter = (
            self.get_term("page-range-delimiter").single
            if variable.startswith("page")
            else None
        )
        range_delimiter = page_range_delimiter or self.unicode_character(
            "EN DASH"
        )
        ...
        def format_number_or_range(item):
            try:
                first, last = (
                    number.strip()
                    for number in item.replace(en_dash, "-").split("-")
                )
            except ValueError:
            ...
            if variable == "page-first":
                return first
            last = self.format_number(
                self._format_last_page(first, last)
                if variable == "page"
                else last
            )
        ...
        return join(
            (
                join(
                    (
                        format_number_or_range(item)
                        for item in comma_item.split("&")
                    ),
                    amp_delimiter,
                )
                for comma_item in str(value).split(",")
            ),
            delimiter=", ",
        )

    def _format_last_page(self, first, last):  # noqa: C901
        def find_common(first, last):
            count = 0
            for count, (first_item, last_item) in enumerate(
                zip(first, last, strict=False)
            ):
        ...
        if range_format == "chicago":
            ...
            if (
                first_number < 100  # noqa: PLR2004
                or first_number % 100 == 0
                or (len(first) >= 4 and common < 2)  # noqa: PLR2004
            ):
...
class Text(
    ...
    def _term(self, item):
        ...
        if plural:
            text = term.multiple if term is not None else None
        else:
            text = term.single if term is not None else ""
...
class Date(CitationStylesElement, Parent, Formatted, Affixed, Delimited):
    ...
    def render_date_range(self, date_range, show_parts=None, context=None):
        ...
        if date_range.end.is_nil():
            same = None
            diff_begin = self.render_single_date(
                date_range.begin, show_parts, context
            )
            diff_end = ""
        else:
            if date_range.begin.year == date_range.end.year:
                ...
                try:
                    if (
                        "month" in show_parts
                        and date_range.begin.month == date_range.end.month
                    ):
                        ...
                        try:
                            if (
                                "day" in show_parts
                                and date_range.begin.day == date_range.end.day
                            ):
            ...
            same = self.render_single_date(
                date_range.end, same_show_parts, context
            )
            diff_begin = self.render_single_date(
                date_range.begin, show_parts, context
            )
            diff_end = self.render_single_date(
                date_range.end, show_parts, context
            )
        ...
        diff = (
            diff_begin.rstrip() + self.unicode_character("EN DASH") + diff_end
        )

        return context.join([diff, same.rstrip()]) if same else diff

    def process(
        self, item, variable=None, show_parts=None, context=None, **kwargs
    ):
        if variable is None:
            variable = self.get("variable")
        ...
        if not self.is_locale_date() and form is not None:
            ...
            return localized_date.render(
                item, variable, show_parts=show_parts, context=self
            )
        ...
        if not date_or_range:
            ...
            text = self.render_date_range(date_or_range, show_parts, context)
        else:
            text = self.render_single_date(date_or_range, show_parts, context)
        if text is not None:
            style_context = context if self.is_locale_date() else self
...
class Date_Part(  # noqa: N801
    CitationStylesElement, Formatted, Affixed, TextCased, StrippedPeriods
):
    def process(self, date, context=None):
        ...
        if name == "day":
            text = self._handle_day(context, date)
        elif name == "month":
            text = self._handle_month(context, date)
        elif name == "year":
            text = self._handle_year(context, date)

        return text

    def _handle_year(self, context, date):
        form = self.get("form", "long")
        if form == "long":
            text = str(abs(date.year))
            if date.year < 0:
                text += context.get_term("bc").single
            elif date.year < AD_THRESHOLD:
                text += context.get_term("ad").single
        elif form == "short":
            text = str(date.year)[-2:]
        return text

    def _handle_month(self, context, date):
        form = self.get("form", "long")
        try:
            index = date.month
            term = "month"
            ...
            term = "season"
        if form == "long":
            text = context.get_term(f"{term}-{index:02}").single
        elif form == "short":
            term = context.get_term(f"{term}-{index:02}", "short")
            text = term.single
        else:
            assert term == "month"
            if form == "numeric":
    ...
    def _handle_day(self, context, date):
        form = self.get("form", "numeric")
        if (
            form == "ordinal"
            and self.get_locale_option("limit-day-ordinals-to-day-1").lower()
            == "true"
            and date.day > 1
        ):
            form = "numeric"
...
class Number(
    ...
    def format_number(self, number):
        ...
        if form == "numeric":
            text = str(number)
        elif form == "ordinal" or (
            form == "long-ordinal" and number > ORDINAL_THRESHOLD
        ):
            text = to_ordinal(number, self)
        elif form == "long-ordinal":
            text = self.get_term(f"long-ordinal-{number:02}").single
        elif form == "roman":
...
class Names(CitationStylesElement, Parent, Formatted, Affixed, Delimited):
    ...
    def process(self, item, names_context=None, context=None, **kwargs):
        ...
        try:
            ed_trans = (
                set(roles) == {"editor", "translator"}
                and item.reference.editor == item.reference.translator
                and self.get_term("editortranslator").getchildren()
            )
            if ed_trans:
                roles = ["editor"]
        except VariableError:
        ...
        text = self._parse_role(
            context, ed_trans, item, kwargs, names_context, output, roles
        )

        text = self._check_types(context, item, kwargs, output, text)

        try:
            return text
        except NameError:
            raise VariableError from NameError

    def _check_types(self, context, item, kwargs, output, text):
        if output:
            try:
                total = sum(output)
            except TypeError:
        ...
        for role in roles:
            if role in item.reference:
                ...
                try:
                    new_role = role
                    if ed_trans:
                        new_role = "editortranslator"
                    label_element = names_context.label

                    label = label_element.render(
                        item, new_role, plural, **kwargs
                    )
...
class Name(CitationStylesElement, Formatted, Affixed, Delimited):
    ...
    def process(
        self, item, variable, context=None, sort_options=None, **kwargs
    ):
        def get_option(name):
        ...
        et_al_truncate = (
            len(names) > 1 and et_al_min and len(names) >= et_al_min
        )
    ...
    def _handle_et_al(  # noqa: PLR0913
        self,
        and_,
        and_term,
        delimiter,
    ...
    def get_options(self, get_option):
        and_ = get_option("and")
...
class Name_Part(  # noqa: N801
    CitationStylesElement, Formatted, Affixed, TextCased
):
...
class Et_Al(CitationStylesElement, Formatted, Affixed):  # noqa: N801
    def process(self):
        return self.get_term("variable")
        ...
        for child in self.getchildren():
            try:
                if isinstance(child, Names) and child.name is None:
                    ...
                    text = child.render(
                        item, names_context=names, context=context, **kwargs
                    )
                else:
...
class Label(
    CitationStylesElement, Formatted, Affixed, StrippedPeriods, TextCased
):
    def calls_variable(self):
        return self.get("variable") == "locator"

    def process(
        self, item, variable=None, plural=None, context=None, **kwargs
    ):
        if variable is None:
            variable = self.get("variable")
        form = self.get("form", "long")
        ...
        if (
            plural_option == "contextual" and plural
        ) or plural_option == "always":
            text = term.multiple
        else:
            text = term.single if term is not None else ""
        ...
        for child in self.iterchildren():
            ...
            try:
                ...
                if child_text is not None:
                    ...
                    variable_rendered = (
                        variable_rendered or child.calls_variable()
                    )
            except VariableError:
                pass
...
class If(CitationStylesElement, Parent):
    ...
    def _type(self, item):
        return [
            typ.lower() == item.reference.type
            for typ in self.get("type").split()
        ]

    def _variable(self, item):
        variables = [
            var.replace("-", "_") for var in self.get("variable").split()
        ]
        output = []
    ...
    def _is_numeric(self, item):
        ...
        return [
            var.replace("-", "_") in item.reference
            and numeric_match(str(item.reference[var.replace("-", "_")]))
    ...
    def _locator(self, item):
        return [
            item.has_locator and var.replace("-", " ") == item.locator.label
    ...
    def _position(self, item, context):
        ...
        possibly_ibid = (
            ...
            and (
                item.citation is last_cite.citation
                or len(last_cite.citation.cites) == 1
            )
        )
        results = []
        self._parse_position(
            already_cited, cites, item, last_cite, possibly_ibid, results
        )
        return results

    def _parse_position(  # noqa: PLR0913
        self,
        already_cited,
        cites,
        item,
        ...
        for position in self.get("position").split():
            ...
            if position == "first":
                ...
                result = (item.has_locator and not last_cite.has_locator) or (
                    item.has_locator
                    and last_cite.has_locator
                ...
                for cite in reversed(cites):
                    if item.key == cite.key and citations <= max_distance:
                        result = True
                        break
                    if cite.citation is not last_citation:
                        citations += 1
                        last_citation = cite.citation
            results.append(result)


class Else_If(If, CitationStylesElement):  # noqa: N801
    pass
